...
function validateGestanteFileContent(content: string, selectedIps: string): ValidationError[] {
  ...
  dataRows.forEach((row, rowIndex) => {
    ...
    const ipsPrimaria = columns[28];
    if (ipsPrimaria && ipsPrimaria.trim() !== selectedIps) {
        errors.push({
            location: `Fila ${rowIndex + 2}, Columna 29`,
            type: 'Valor no válido',
            description: `El nombre de la IPS Primaria "${ipsPrimaria}" no coincide con la IPS seleccionada "${selectedIps}".`,
        });
    }
  });
...
function validateRcvFileContent(content: string, selectedIps: string): ValidationError[] {
  const errors: ValidationError[] = [];
  const rows = content.split(/\r?\n/);

  // Header is in the first 3 rows, data starts from 4th
  ...
  dataRows.forEach((row, rowIndex) => {
    ...
    const ipsSeguimiento = columns[22]; // Column 23, "NOMBRE DE LA IPS QUE HACE SEGUIMIENTO"
    if (ipsSeguimiento && ipsSeguimiento.trim() !== selectedIps) {
        errors.push({
            location: `Fila ${rowIndex + 4}, Columna 23`,
            type: 'Valor no válido',
            description: `La IPS de seguimiento "${ipsSeguimiento}" no coincide con la IPS seleccionada "${selectedIps}".`,
        });
    }
  });
...
export async function validateValidadorFile(file: File, selectedIps: string, validatorType: 'gestante' | 'rcv') {
    if (!selectedIps) {
        return { errors: [], message: 'Debe seleccionar una IPS antes de validar un archivo.' };
    }
    ...
    try {
        ...
        const validationErrors = validatorType === 'gestante' 
            ? validateGestanteFileContent(fileContentForValidation, selectedIps)
            : validateRcvFileContent(fileContentForValidation, selectedIps);
...
export async function checkForExistingFile(year: string, month: string, ips: string, validatorType: 'gestante' | 'rcv'): Promise<string | null> {
    if (!year || !month || !ips) {
        return null;
    }
    ...
    try {
        ...
        const existingFile = filesInDir.find(file => file.startsWith(`${ips}-`));
        return existingFile || null;
    } catch (error) {
        // Directory doesn't exist, so no file to overwrite
        return null;
    }
}


export async function uploadValidadorFile(formData: FormData) {
  ...
  if (!ips) {
    return { success: false, error: 'IPS not provided.' };
  }
   if (!validatorType) {
  ...
  try {
    ...
    const finalFileName = `${ips}-${intermediateFileName}`;
