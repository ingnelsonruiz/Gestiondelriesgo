...
function getMonthAndYearFromContent(file: File, fileContent: string | ArrayBuffer): { month: string; year: string } | null {
    if (file.type.includes('sheet')) {
        const workbook = XLSX.read(fileContent, { type: 'binary', cellDates: true, dateNF: 'dd/mm/yyyy' });
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];

        const range = XLSX.utils.decode_range(worksheet['!ref'] || 'A1:A1');
        // The date for gestante is on the first row (index 0)
        const targetRow = 0; 
        if (range.e.r < targetRow) return null;

        for (let C = range.s.c; C <= range.e.c; ++C) {
            const address = XLSX.utils.encode_cell({ r: targetRow, c: C });
        ...
        const rows = (fileContent as string).split(/\r?\n/);
        // The date for gestante is on the first row (index 0)
...
export default function GestanteValidator() {
  ...
  const processFile = async (selectedFile: File) => {
    ...
    try {
        const result = await validateValidadorFile(selectedFile, selectedIps, 'gestante');
  ...
  const handleUploadConfirmation = async () => {
    ...
    const existingFile = await checkExistingValidadorFile(year, month, selectedIps, 'gestante');
  ...
  const handleLoadClick = async () => {
    ...
    try {
      ...
      formData.append('validatorType', 'gestante');
      if (fileToOverwrite) {
  ...
  return (
    <div className="space-y-8 mt-4">
        <div className="grid gap-4">
          <Label htmlFor="ips-select">Seleccione la IPS</Label>
          <Select onValueChange={setSelectedIps} value={selectedIps}>
              <SelectTrigger id="ips-select" className="w-full">
